...
func (repository *ReservationRepository) ListEligibleSellers(ctx context.Context, loteoID string, actorAuthProviderID string, scope gateway.ReservationScope) ([]domain.SellerOption, error) {
	var actor *string
	if actorAuthProviderID != "" {
		actor = &actorAuthProviderID
	}
	rows, err := repository.pool.Query(ctx, `
	...
	`, loteoID, scope.AssigneeAuthProviderID, scope.ByAgencyAssignment, scope.ForSale, actor)
	if err != nil {
