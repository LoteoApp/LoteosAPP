...
func (fake *userRepositoryFake) FindByAuthProviderID(context.Context, string) (domain.Usuario, error) {
	return domain.Usuario{}, nil
}

func (fake *userRepositoryFake) UpdateProfile(_ context.Context, authProviderID, nombre, apellido string) (domain.Usuario, error) {
...
func (fake *identityProviderFake) CreateUser(_ context.Context, email, rol string) (string, string, error) {
	...
	return fake.authProviderID, fake.tempPass, nil
}

func (fake *identityProviderFake) DeleteUser(_ context.Context, authProviderID string) error {
	fake.deleteCalls++
...
func TestUserServiceCreateUserHappyPath(t *testing.T) {
	...
	repository := &userRepositoryFake{}
	identity := &identityProviderFake{authProviderID: "sb-123", tempPass: "temp-pass-123"}
	service := NewUserService(repository, identity)
	...
	if usuario.AuthProviderID != "sb-123" {
		t.Errorf("CreateUser() auth provider id = %q, want %q", usuario.AuthProviderID, "sb-123")
	}
...
func TestUserServiceCreateUserCompensatesWhenPersistenceFails(t *testing.T) {
	...
	if identity.deletedUserID != "sb-123" {
		t.Errorf("CreateUser() compensated user id = %q, want %q", identity.deletedUserID, "sb-123")
	}
}
...
func TestUserServiceCreateUserReturnsOriginalErrorWhenCompensationAlsoFails(t *testing.T) {
	...
	persistErr := errors.New("insert failed")
	repository := &userRepositoryFake{createErr: persistErr}
	identity := &identityProviderFake{authProviderID: "sb-123", deleteErr: errors.New("delete failed")}
	service := NewUserService(repository, identity)
...
func TestUserServiceCompleteProfileHappyPath(t *testing.T) {
	...
	usuario, err := service.CompleteProfile(context.Background(), "sb-123", "Ana", "Gómez")
	if err != nil {
...
func TestUserServiceCompleteProfileRejectsEmptyFields(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, err := service.CompleteProfile(context.Background(), "sb-123", test.nombre, test.apellido)
...
func TestUserServiceCompleteProfilePropagatesRepositoryError(t *testing.T) {
	...
	_, err := service.CompleteProfile(context.Background(), "sb-123", "Ana", "Gómez")
