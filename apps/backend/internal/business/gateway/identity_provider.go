...
// IdentityProvider abstracts the identity provider (Keycloak) so the
// business layer creates and removes accounts without depending on a
// concrete adapter.
type IdentityProvider interface {
	...
	CreateUser(ctx context.Context, email, rol string) (keycloakID, temporaryPassword string, err error)

	// DeleteUser removes the account. Used to compensate a CreateUser call
	// whose local persistence step failed afterwards.
	DeleteUser(ctx context.Context, keycloakID string) error
}
